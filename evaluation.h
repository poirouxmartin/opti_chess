
// Evaluateur de position (heuristiques)
class Evaluator {
    public:


    // Paramètres d'évaluation

    // Coefficiants des heuristiques
    float _piece_value = 1;
    float _piece_activity = 0.1;
    float _piece_positioning = 0.025;
    float _random_add = 0.25;
    float _bishop_pair = 0.9;
    float _castling_rights = 0.5;


    // Valeur des pièces
    float _pawn_value_begin = 1;
    float _pawn_value_end = 1.25;
    float _knight_value_begin = 3.1;
    float _knight_value_end = 2.6;
    float _bishop_value_begin = 3.3;
    float _bishop_value_end = 3.1;
    float _rook_value_begin = 4.6;
    float _rook_value_end = 5.8;
    float _queen_value_begin = 8.9;
    float _queen_value_end = 8.9;
    float _king_value_begin = 1000000;
    float _king_value_end = 1000000;


    // Positionnement des pièces

    int _pos_pawn_begin[8][8]       {   {   0,    0,    0,    0,    0,    0,    0,    0},
                                        {  78,   83,   86,   93,  102,   82,   85,   90},
                                        {  37,   29,   21,   54,   50,   31,   44,   37},
                                        { -17,   16,   20,   65,   64,   30,   35,   13},
                                        { -26,    3,   30,   49,   50,  -10,  -20,  -23},
                                        { -22,    9,   10,   20,   25,  -40,   10,  -19},
                                        { -31,    8,  -27,    0,   -6,   20,    3,  -31},
                                        {   0,    0,    0,    0,    0,    0,    0,    0}   };

    int _pos_pawn_end[8][8]         {   {   0,    0,    0,    0,    0,    0,    0,    0},
                                        {  78,   83,   86,   73,  102,   82,   85,   90},
                                        {  57,   49,   41,   54,   50,   41,   44,   57},
                                        {  27,   16,   30,   45,   44,   30,   35,   33},
                                        { -26,    3,   15,   29,   30,  -10,  -20,   13},
                                        { -22,    9,    5,   10,   15,  -40,    3,  -19},
                                        { -31,    8,   -7,  -27,  -26,   20,    3,  -31},
                                        {   0,    0,    0,    0,    0,    0,    0,    0}   };

    int _pos_knight_begin[8][8]     {   { -66,  -53,  -75,  -75,  -10,  -55,  -58,  -70},
                                        {  -3,   -6,  100,  -36,    4,   62,   -4,  -14},
                                        {  10,   67,    1,   74,   73,   27,   62,   -2},
                                        {  24,   24,   45,   37,   43,   41,   25,   17},
                                        {  -1,    5,   31,   21,   22,   35,    2,    0},
                                        { -18,   10,   13,   22,   18,   25,   14,  -14},
                                        { -23,  -15,    2,    5,    2,    0,  -23,  -20},
                                        { -74,   -5,  -26,  -24,  -19,  -30,  -20,  -69}   };

    int _pos_knight_end[8][8]       {   { -66,  -53,  -75,  -75,  -10,  -55,  -58,  -70},
                                        {  -3,   -6,  100,  -36,    4,   62,   -4,  -14},
                                        {  10,   67,    1,   74,   73,   27,   62,   -2},
                                        {  24,   24,   45,   37,   43,   41,   25,   17},
                                        {  -1,    5,   31,   21,   22,   35,    2,    0},
                                        { -18,   10,   13,   22,   18,   15,   14,  -14},
                                        { -23,  -15,    2,    0,    2,    0,  -23,  -20},
                                        { -74,  -25,  -26,  -24,  -19,  -30,  -20,  -69}   };

    int _pos_bishop_begin[8][8]     {   { -59,  -78,  -82,  -76,  -23, -107,  -37,  -50},
                                        { -11,   20,   35,  -42,  -39,   31,    2,  -22},
                                        {  -9,   39,  -32,   41,   52,  -10,   28,  -14},
                                        {  25,   17,   20,   34,   26,   25,   15,   10},
                                        {  13,   10,   17,   23,   17,   16,    0,    7},
                                        {  14,   25,   24,   15,    8,   25,   20,   15},
                                        {  19,   25,   11,    6,    7,    6,   25,   16},
                                        {  -7,    2,  -15,  -12,  -14,  -20,  -10,  -10}   };

    int _pos_bishop_end[8][8]       {   { -59,  -78,  -82,  -76,  -23, -107,  -37,  -50},
                                        { -11,   20,   35,  -42,  -39,   31,    2,  -22},
                                        {  -9,   39,  -32,   41,   52,  -10,   28,  -14},
                                        {  25,   17,   20,   34,   26,   25,   15,   10},
                                        {  13,   10,   17,   23,   17,   16,    0,    7},
                                        {  14,   25,   24,   15,    8,   25,   20,   15},
                                        {  19,   25,   11,    6,    7,    6,   25,   16},
                                        {  -7,    2,  -15,  -12,  -14,  -20,  -10,  -10}   };

    int _pos_rook_begin[8][8]       {   {  35,   29,   33,    4,   37,   33,   56,   50},
                                        {  55,   29,   56,   67,   55,   62,   34,   60},
                                        {  19,   35,   28,   33,   45,   27,   25,   15},
                                        {   0,    5,   16,   13,   18,   -4,   -9,   -6},
                                        { -28,  -35,  -16,  -21,  -13,  -29,  -46,  -30},
                                        { -42,  -28,  -42,  -25,  -25,  -35,  -26,  -46},
                                        { -53,  -38,  -31,  -26,  -29,  -43,  -44,  -53},
                                        { -30,  -24,    5,   20,   30,    5,  -31,  -32}   };

    int _pos_rook_end[8][8]         {   {  35,   29,   33,    4,   37,   33,   56,   50},
                                        {  55,   29,   56,   67,   55,   62,   34,   60},
                                        {  19,   35,   28,   33,   45,   27,   25,   15},
                                        {   0,    5,   16,   13,   18,   -4,   -9,   -6},
                                        { -28,  -35,  -16,  -21,  -13,  -29,  -46,  -30},
                                        { -42,  -28,  -42,  -25,  -25,  -35,  -26,  -46},
                                        { -53,  -38,  -31,  -26,  -29,  -43,  -44,  -53},
                                        { -30,  -24,    5,   20,   30,    5,  -31,  -32}   };

    // à changer... 
    int _pos_queen_begin[8][8]      {   { -50,  -50,  -50,  -50,  -50,  -50,  -50,  -50},
                                        { -70,  -60,    0,    0,    0,   40,   40,   40},
                                        { -40,  -40,  -40,  -40,  -40,  -40,  -40,  -40},
                                        { -20,  -40,  -40,  -60,  -60,  -40,  -40,  -20},
                                        { -40,  -40,  -40,  -50,  -50,  -40,  -40,  -40},
                                        {   0,  -40,  -20,   20,    0,   20,    0,    0},
                                        { -40,  -20,   10,   10,   10,    0,  -20,  -40},
                                        { -80,  -40,  -20,   20,   10,  -40,  -40,  -80}   };

    int _pos_queen_end[8][8]        {   {   6,    1,   -8,  -20,   69,   24,   88,   26},
                                        {  14,   32,   60,  -10,   20,   76,   57,   24},
                                        {  -2,   43,   32,   60,   72,   63,   43,    2},
                                        {   1,  -16,   22,   40,   25,   20,  -13,   -6},
                                        { -14,  -15,    5,   20,   15,  -10,  -20,  -22},
                                        { -30,   -6,  -13,  -11,  -16,  -11,  -16,  -27},
                                        { -36,  -18,    0,  -19,  -15,  -15,  -21,  -38},
                                        { -39,  -30,  -31,  -20,  -31,  -36,  -34,  -42}   };

    int _pos_king_begin[8][8]       {   {   4,   54,   47,  -99,  -99,   60,   83,  -62},
                                        { -32,   10,   55,   56,   56,   55,   10,    3},
                                        { -62,   12,  -57,   44,  -67,   28,   37,  -31},
                                        { -55,   50,   11,   -4,  -19,   13,    0,  -49},
                                        { -55,  -43,  -52,  -28,  -51,  -47,   -8,  -50},
                                        { -47,  -42,  -43,  -79,  -64,  -32,  -29,  -32},
                                        {  -4,    3,  -14,  -50,  -57,  -18,   13,    4},
                                        {  37,   40,   10,  -64,  -50,  -10,   60,   48}   };

    int _pos_king_end[8][8]         {   { -40,  -30,  -20,  -10,  -10,  -20,  -30,  -40},
                                        { -30,   37,   50,   50,   50,   50,   37,  -30},
                                        {   0,   45,   60,   75,   75,   60,   45,    0},
                                        {   0,   50,   60,  100,  100,   60,   50,    0},
                                        { -10,   50,   50,  100,  100,   50,   50,  -10},
                                        { -20,   50,   50,   50,   50,   50,   50,  -20},
                                        { -40,   25,   25,   40,   40,   25,   25,  -40},
                                        { -60,  -50,  -40,  -30,  -30,  -40,  -50,  -60}   };


    // Constructeur par défaut
    Evaluator();



};