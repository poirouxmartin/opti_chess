...
- **Baseline/last discipline (no checkout of old commits): at every breakthrough, freeze a baseline build and keep it intact until the next breakthrough.** Sibling worktree `../opti_chess_baseline` pinned on the baseline commit (no cmake configure available for VS18: `build/release/*.vcxproj` copied over with source paths repointed, ZERO_CHECK refs removed, gtest/raylib linked from main `build/release` libs). Snapped binaries in `build/bench_bin/` (`opti_chess_tests_baseline.exe` = frozen, `opti_chess_tests_last.exe` = re-snapped from master tip after every validated change). Compare dev against the frozen baseline: `Puzzle.LichessBenchmark5000` NODES-500 (gate, ~20 min) then NODES-2000 (~80 min), SEED-42, CSVs in `build/bench_bin/`, McNemar significant iff χ²>3.84. Promote to new baseline only when a master is validated (re-pin worktree + rebuild + re-snap).
