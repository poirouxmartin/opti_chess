...
- **No CI and no CTest.** Validation is the interactive `T` suite plus manual GUI checks. The `CMakeLists.txt` still carries a GoogleTest target, but it is scaffolding rather than a suite — it compiles the interactive tests against GoogleTest and does not link, so it is `OFF` by default (`-DBUILD_TESTS=ON` breaks the build until it is rewritten).
