...
// (both sides in the search chose to return).
...
void Node::explore_new_move(BoardBuffer* board_buffer, Evaluator* eval, double alpha, double beta, double gamma, int quiescence_depth, Network* network, PositionHistory *path_history) {
	...
	if (!child->_fully_explored) {
		...
		int child_depth = quiescence_depth;
		...
		if (g_selective_deepening || g_adaptive_quiescence) {
			...
			if (g_adaptive_quiescence && child_depth > 0) {
				...
				const uint8_t capt = _board->_array[move.end_row][move.end_col];
				const bool is_capture = capt != none && mover != none;

				static constexpr int piece_vals_see[13] = { 0, 100, 320, 330, 500, 900, 10000, 100, 320, 330, 500, 900, 10000 };
				const int eval_delta = _board->_player ? (child->_static_evaluation._value - _static_evaluation._value) : (_static_evaluation._value - child->_static_evaluation._value);

				if (is_capture) {
					if (piece_vals_see[capt] < piece_vals_see[mover]) {
						if      (eval_delta < -500)  child_depth = 0;
						else if (eval_delta < -100)  child_depth = min(child_depth, 4);
					}
				}
				else {
					if      (eval_delta < -500)  child_depth = 0;
					else if (eval_delta < 0)     child_depth = min(child_depth, 4);
				}
			}
		}
...
int Node::quiescence(BoardBuffer* board_buffer, Evaluator* eval, int depth, double search_alpha, double search_beta, int alpha, int beta, Network* network, bool evaluate_threats, int beta_margin, PositionHistory *path_history) {
	...
	// Capture-only local (no shared mutation)
	Move q_moves[max_moves];
	int q_count = 0;
	for (int _qi = 0; _qi < _board->_got_moves; ++_qi) {
		const Move &qm = _board->_moves[_qi];
		if (in_check || qm.is_capture() || qm.is_promotion() || qm.is_check() || qm.is_checkmate()) q_moves[q_count++] = qm;
	}
	...
	for (int i = 0; i < q_count; i++) {
		...
		if (new_depth <= 0 && !in_check) {
			...
			if (child == nullptr) {
				...
				// An adopted shared node (child->_board != new_board) is never
				// touched: the draw is path-local, the node is not.
				if (q_is_rep_draw && child != nullptr && child->_board == new_board) {
					init_terminal_draw_child(child, new_board, eval, network);
				}
			}
